    ...
    <div v-if="listItem && listItem.id" class="card col-md-6 col-xl-4" :id="listItem.id">
        <div class="thumbnailActions">
            <a :href="url+'/cms/page/'+page.url+'/'+listItem.page_list_id+'/'+listItem.id+'/edit'"
               ...
               v-if="listItem.hasOptions"><i
                class="fa fa-edit" style="color: #ffffff"></i></a>
            <button class="btn btn-danger" @click="emitDeleteItem"><i class="fa fa-trash"></i></button>
        </div>
        <div class="card-body"><!--| capitalize-->
            <h3 class="card-title" v-if="listItem && listItem.title || (product && (product.title || product.name))">
                {{$options.filters.striphtml(listItem.title).trim().length?listItem.title:product.title?product.title:product.name
                | striphtml | capitalize}}</h3>
