...
<div class="owl-carousel owl-theme owl-hide beforeLoad" id="mainSlideOwl">
    @foreach ($lists['main_slide'] as $headItem)
        @isset($headItem->id)
            @php($headEntityItem = $headItem->entityItem)
            <div class="owlHomeItem d-flex d-lg-block owl-hide"
                 style="height: 86vh; overflow: hidden; max-height: 600px;">
                <img
                    @isset($headItem->options->slide_background->value)
                    data-src="{{img('_img/head-slide/backgrounds/'.$headItem->options->slide_background->value)}}"
                    src=""
                    @endisset
                    ...
                    width="100%"
                >
                <div class="col p-5 p-lg-1 position-absolute"
                     style="background-color: rgba(255, 255, 255, 0.4);top: 0;">
                    <div
                        class="container-1112 justify-content-around align-items-center align-items-lg-stretch flex-column d-flex d-lg-grid">
                        ...
                        <div class="p g-col-1" dir="ltr" style="font-size: 11pt;">
                            <div
                                class="d-none d-md-block">{!! !empty($headItem->options->slide_description->value)?$headItem->options->slide_description->value:$headEntityItem->description !!}</div>
                            <a class="shopNow m-auto m-lg-0 mt-lg-5"
                               href="{{url("shop/$headEntityItem->c_url/$headEntityItem->url")}}">Shop
                                Now <img src="{{img('_img/head-slide/shop-right-arrow.png')}}" alt=""
